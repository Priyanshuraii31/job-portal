...
function Applicants() {
  ...
  return (
    <div className="jobs">
      ...
      {applications.length === 0 && <p>No applicants for this job yet.</p>}
      ...
      {applications.map((app) => (
        <div className="job-card" key={app._id}>
          ...
          <button className="primary-btn" onClick={() => updateStatus(app._id, "Selected")}>
            Select
          </button>

          <button className="delete-btn" onClick={() => updateStatus(app._id, "Rejected")}>
            Reject
          </button>
        </div>
      ))}
